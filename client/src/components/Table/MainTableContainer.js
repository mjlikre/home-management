...
import { getSummary } from "../../actions/operations";
    ...
    if (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        if (!details.hasOwnProperty(data[i].client_name)) {
          ...
          details[data[i].client_name].amount += data[i].amount;
          details[data[i].client_name].quantity += data[i].quantity;
        }
      }
      ...
      <div style={{ opacity: "0.8" }}>
        <div className="header-grid">
          {!mobile ? (
            <h3>
              当前周期现有牛黄：
              {props.summary ? props.summary.data[0][0].quantity : null}克
            </h3>
          ) : (
            <h5>
              现有：{props.summary ? props.summary.data[0][0].quantity : null}克
            </h5>
          )}
...
export default compose(connect(mapStateToProps, { getSummary }))(
  MainTableContainer
);
